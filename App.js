...
3. Luego puedes invitarlo`
      ...
      await db.collection('users').doc(userDoc.id).update({
        farms: firestore.FieldValue.arrayUnion(newFarm)
      });
...
// ============ CÓDIGO CORREGIDO - LÍNEAS 8600-8810 ============
...
export default function App() {
  ...
  console.log('✅ useState inicializado');
  
  const [loading, setLoading] = useState(true);
  console.log('✅ loading state inicializado');
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth().onAuthStateChanged(async (user) => {
      console.log('👤 Auth state changed:', user ? `Usuario: ${user.email}` : 'Sin usuario');
        ...
        try {
          ...
          if (!premium) {
            try {
              ...
              const getAdsWatchedToday = async () => 0;
const cleanExpiredCredits = async () => {};
        ...
        try {
          const adminUserDoc = await db.collection('users').doc(user.uid).get();
